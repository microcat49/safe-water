...
function archive_wqmodel($repost) {
		...
		set_time_limit(250);
		check_keys();
		print("<br>Initializing logfile...<br>");
...
// NOTE NIC STOPPED HERE 20200407
// O	Need to calculate HOURS parameter some other way, it is pulled from the
//		database that doesn't exist anymore.
// O 	Francois says the flow data is from UGSS, need to coordinate with
//		@Daniel Reeves about how to get those data.
function boatingmodel() {
...
function archive_boatingmodel() {
		...
		$avgcount = 1 ;
		$trgthour = mktime(0,0,0,0,0,0) ; // NOTE: this will be current time
		$site = "CharlesCB";
		...
		while($row = $statement->fetch_assoc()){
			...
			// NOTE: accumulates time measurements into arrays
			...
			if (($thishour >= $starttime) && ($thishour <= $endtime)) {
				...
				if ($thishour == $trgthour) {
					...
					} // NOTE: end if in the future but still not current time
					$trgthour = $thishour ;
...
function read_boatingmodel2($days) {
	// Modified by Nic Webb 3/24/20
	// To test whether HOBOLINK connection could be used
	// to get data.
	...
	//$query = "ReST_Query" ; // original
	$query="code_for_boston_export"; // Daniels' workaround

	//INSERTED FROM archive_repeatingcode.php 85:99
	//use standard ReST url
	...
	// Modified 3/31/20 by Nic Webb
	// parses HOBOlink results into two data structures:
	//		$data[column_name] = [array of data for column]
	//		^^^^^^ this is used for averaging calculations over time
	//		$rowdata[row_number] = [array of all columns for that row]
	//		^^^^^^^^ keyed by row index for easy table display and debug
	// HOBOlink data comes in two sections separated by "------"
	// the first section is YAML formatted metadata and the second
	// is csv formatted data.

	// Split curl response into YAML and csv
	$parsed = preg_split("/\-\-\-\-\-\-\-\-\-\-\-\-/",$curl_response);
	$metadata = $parsed[0]; // YAML metadata
	$pre_data = $parsed[1]; // actual data from HOBOlink, to be processed

	// split into rows of strings
	$pre_data = preg_split("/\n/",$pre_data);
	array_shift($pre_data); // first element is empty...
	array_pop($pre_data); // last element is empty...

	// get headers and remove them from $pre_data
	$headers = str_getcsv( array_shift($pre_data) );

	// initialize data storage structures
	$data = array(); // keyed by column, array of column values
	$rowdata = array(); // keyed by row index, array of rows

	// initialize columns in $data as arrays so we can push
	for ( $i=0; $i<count($headers); $i++ ) {
		$data[$headers[$i]] = array();
	}

	// parse into both data structures
	for ( $ri=0; $ri<count($pre_data); $ri++ ) { // $ri is "row index" counter
		$row_ar = str_getcsv($pre_data[$ri]); // parse row string into array
		$rowdata[$ri] = array(); // initialize row in $rowdata
		for ( $i=0; $i<count($headers); $i++ ) {
			if (isset($row_ar[$i])) {
				// store in appropriate column array of $data
				array_push($data[$headers[$i]],$row_ar[$i]);

				// store in appropriate row of $rowdata
				array_push($rowdata[$ri],$row_ar[$i]);
			}
		}
	}
	...
	// next step is to perform the model calculations and plug into the flag
	// display by returning appropriate results.

	exit();
}


// ORIGINAL READBOATINGMODEL READ_BOATINGMODEL
function read_boatingmodel($days) {
		...
		$statement = $wqdb->query("SELECT 
										...
										WHERE location='$loc' 
											AND datetime>=TIMESTAMP('$startdatestring') 
											AND datetime<=TIMESTAMP('$enddatestring') 
										ORDER BY datetime");
		//$statement = $wqdb->query("SELECT datetime,watertemp_C,rain_in,raindays,windspeed_mph,flow_cfs,par_uE,li_conc_NBBU_cfu,li_conc_LF_cfu,lg_prb_NBBU_pct,lg_prb_LF_pct,cso_CP,cyano_NewtonYC,cyano_WatertownYC,cyano_CommRowing,cyano_CRCK,cyano_HarvardWeld,cyano_RiversideBC,cyano_CRYC,cyano_UnionBC,cyano_CommBoating,cyano_CRCKKendall FROM $modeltable WHERE location='$loc' AND datetime>=TIMESTAMP('$startdatestring') AND datetime<=TIMESTAMP('$enddatestring') ORDER BY datetime");
		if ($wqdb->error) {
		...
		while($row = $statement->fetch_assoc()){
			...
			$Boating_Model_Entry->Log_R2_set = $row['lg_prb_R2_pct'] ; //2015 update
			$Boating_Model_Entry->Log_R3_set = $row['lg_prb_R3_pct'] ; //2015 update
			$Boating_Model_Entry->Log_R4_set = $row['lg_prb_R4_pct'] ; //2015 update
			$Boating_Model_Entry->Lin_LF_set = $row['li_conc_LF_cfu'] ;
			$Boating_Model_Entry->Log_LF_set = $row['lg_prb_LF_pct'] ;
			$Boating_Model_Entry->cyano_NYC_set = $row['cyano_NewtonYC'] ;
			$Boating_Model_Entry->cyano_WYC_set = $row['cyano_WatertownYC'] ;
			$Boating_Model_Entry->cyano_CR_set = $row['cyano_CommRowing'] ;
			$Boating_Model_Entry->cyano_CRCK_set = $row['cyano_CRCK'] ;
			$Boating_Model_Entry->cyano_HW_set = $row['cyano_HarvardWeld'] ;
			$Boating_Model_Entry->cyano_RBC_set = $row['cyano_RiversideBC'] ;
			$Boating_Model_Entry->cyano_CRYC_set = $row['cyano_CRYC'] ;
			$Boating_Model_Entry->cyano_UBC_set = $row['cyano_UnionBC'] ;
			$Boating_Model_Entry->cyano_CB_set = $row['cyano_CommBoating'] ;
			$Boating_Model_Entry->cyano_CRCKK_set = $row['cyano_CRCKKendall'] ;
